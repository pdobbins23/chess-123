...
class Sprite : public Entity {
  // sprite contains code for a simple OpenGL sprite class that is heirarchical,
  // and can be used to draw a sprite with a texture it is not intended to be a
  // full-featured sprite class, but rather a simple one that can be used for
  // simple games

public:
  Sprite()
      : _parent(nullptr), _location(0, 0), _size(0, 0), _rotation(0), _scale(1),
        _color(1, 1, 1, 1), _localZOrder(0), _highlighted(false) {
  ...
  void setPosition(float x, float y) { _location = ImVec2(x, y); }
  void setPosition(const ImVec2 &point) { _location = point; }
  ...
  void setSize(float x, float y) { _size = ImVec2(x, y); }
  ...
  void paintSprite() {
    if (_size.x > 0.0f && _size.y > 0.0f) {
      ...
      ImGui::Image((void *)(intptr_t)_texture, _size, ImVec2(0, 0),
                   ImVec2(1, 1), _color, highlight);
    }
  }
  ...
  bool isMouseOver(const ImVec2 &mousePos) {
    return (mousePos.x >= _location.x && mousePos.x <= _location.x + _size.x &&
            mousePos.y >= _location.y && mousePos.y <= _location.y + _size.y);
  }
...
protected:
  ...
  ImTextureID _loadTextureFromMemory(const unsigned char *image_data,
