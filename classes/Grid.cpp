...
ChessSquare *Grid::getN(int x, int y) { return getSquare(x, y - 1); }

ChessSquare *Grid::getS(int x, int y) { return getSquare(x, y + 1); }

ChessSquare *Grid::getE(int x, int y) { return getSquare(x + 1, y); }

ChessSquare *Grid::getW(int x, int y) { return getSquare(x - 1, y); }
...
bool Grid::areConnected(int fromX, int fromY, int toX, int toY) {
  ...
  if (_connections.find(fromIndex) != _connections.end()) {
    ...
    return std::find(connections.begin(), connections.end(), toIndex) !=
           connections.end();
  }
...
// Iterator support
void Grid::forEachSquare(
    std::function<void(ChessSquare *, int x, int y)> func) {
  for (int y = 0; y < _height; y++) {
    for (int x = 0; x < _width; x++) {
...
void Grid::forEachEnabledSquare(
    std::function<void(ChessSquare *, int x, int y)> func) {
  for (int y = 0; y < _height; y++) {
    for (int x = 0; x < _width; x++) {
      if (_enabled[y][x]) {
...
void Grid::initializeChessSquares(float squareSize, const char *spriteName) {
  for (int y = 0; y < _height; y++) {
    for (int x = 0; x < _width; x++) {
      ImVec2 position(squareSize * x + squareSize / 2,
                      squareSize * (7 - y) + squareSize / 2);
      _squares[y][x]->initHolder(position, spriteName, x, y);
    }
  }
}

void Grid::initializeSquare(int x, int y, float squareSize,
  ...
  for (int y = 0; y < _height && index < state.length(); y++) {
    for (int x = 0; x < _width && index < state.length(); x++) {
      if (_enabled[y][x]) {
        ...
        // This method just sets the state - games need to create their own
        // pieces when loading from state string based on the piece type
      }
    }
  }
}
