...
// we want the event loop to be elsewhere and calling this class, not called by
// this class but this is fine for tic-tac-toe
//
...
void Game::findDropTarget(ImVec2 &pos) {
  ...
  grid->forEachEnabledSquare([&](ChessSquare *square, int x, int y) {
    ...
    if (square->isMouseOver(pos)) {
      ...
      if (_oldHolder && square->canDropBitAtPoint(_dragBit, pos) &&
          canBitMoveFromTo(*_dragBit, *_oldHolder, *square)) {
        _dropTarget = square;
...
void Game::drawFrame() {
  ...
  // Paint squares
  grid->forEachEnabledSquare(
      [](ChessSquare *square, int x, int y) { square->paintSprite(); });

  // Paint stationary pieces
  grid->forEachEnabledSquare([](ChessSquare *square, int x, int y) {
    if (square->bit() && !square->bit()->getPickedUp() &&
        !square->bit()->getMoving()) {
...
Bit *Game::bitToPlaceInHolder(BitHolder &holder) { return nullptr; }

bool Game::actionForEmptyHolder(BitHolder &holder) { return false; }

bool Game::clickedBit(Bit &bit) { return true; }

bool Game::animateAndPlaceBitFromTo(Bit &bit, BitHolder &src, BitHolder &dst) {
  return false;
}

bool Game::gameHasAI() { return false; }
...
void Game::mouseMoved(ImVec2 &location, Entity *entity) {
  if (_dragBit) {
    // Get the mouse position, and see if we've moved 3 pixels since the
    // mouseDown:
    ImVec2 pos = location;
    if (fabs(pos.x - _dragStartPos.x) >= 12 ||
        fabs(pos.y - _dragStartPos.y) >= 12)
      _dragMoved = true;

    // Move the _dragBit (without animation -- it's unnecessary and slows down
    // responsiveness):
    pos.x += _dragOffset.x;
    pos.y += _dragOffset.y;
...
void Game::mouseUp(ImVec2 &location, Entity *entity) {
  if (!_dragBit) {
    // If no bit was clicked, see if it's a BitHolder the game will let the user
    // add a Bit to:
    if (entity && entity->getEntityType() == Entity::EntityBitHolder) {
      BitHolder *holder = (BitHolder *)entity;
